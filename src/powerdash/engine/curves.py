...
class DailyBaseForwardCurve:
    ...
    def __init__(self, fwd_curves_daily: pd.DataFrame):
        ...
        self._df[COL_DELIV_DATE] = pd.to_datetime(
            self._df[COL_DELIV_DATE]
        ).dt.normalize()
        self._idx = self._df.set_index([COL_VAL_DATE, COL_DELIV_DATE])[
            COL_FWD_BASE
        ].sort_index()

    def get_fwd_base(
        self, val_date: pd.Timestamp, delivery_date: pd.Timestamp
    ) -> float:
        ...
        try:
            ...
            raise KeyError(
                f"Missing fwd_base for val_date={val.date()} delivery_date={d.date()}"
            ) from e
